...
//   (3) deployed context (signal present) + prod host        → allowed silently
...
test('(3) deployed context (SYNTHWATCH_LOCATION, as bicep sets on every index.ts job) + prod host → allowed', () => {
  const v = prodGuardVerdict({ DATABASE_URL: PROD_URL, SYNTHWATCH_LOCATION: 'eastus2' });
  assert.deepEqual(v, { allowed: true, reason: 'deployed' });
});

test('(3b) deployed context via ACA platform metadata (CONTAINER_APP_JOB_NAME) → allowed', () => {

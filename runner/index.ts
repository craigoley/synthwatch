...
import { makeRedactor, IDENTITY_REDACTOR, tracePersistPlan, sensitiveErrorMessage } from './redact.js';
...
async function runOne(check: Check): Promise<void> {
  ...
  // (built-in token denylist + declared patterns); genericise the run-level error_message (a Playwright
  // native error can echo DOM values/PII) — keep only the safe status + static step name. A warn's
  // perf-breach message is generated by us (no PII) so it's left intact. Non-sensitive: unchanged.
  const sensitive = check.sensitive;
  const redact = sensitive ? makeRedactor(check.redact_patterns) : IDENTITY_REDACTOR;
  const persist = tracePersistPlan(sensitive, status);
  if (sensitive && (status === 'fail' || status === 'error')) {
    errorMessage = sensitiveErrorMessage(status, outcome.failedStep);
  }
